...
The warnings buffer is super nice but it can be annoying that it pops up into view. I prefer to just open the buffer when I want to deal with it
#+begin_src emacs-lisp
...
#+end_src
**** modus-themes
Another of prot's great themes, these are built-in
#+begin_src emacs-lisp
  (use-package modus-themes
    :straight t)
#+end_src
**** doric-themes
Also from Prot, a more minimal set of themes
#+begin_src emacs-lisp
  ...
  (use-package denote-silo
    ...
    (setq denote-silo-directories
          (list denote-directory
                (expand-file-name "~/Notes/TheIronreachAccord")
  		)))
#+end_src
*** Calendar
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package rst
    :hook ((rst-mode . display-line-numbers-mode)
...
#+begin_src emacs-lisp
  (use-package cargo
    :config
    (setq cargo-process--command-test "nextest run --all-features --color always --lib")
...
#+begin_src emacs-lisp
  (use-package general
    ...
    (general-define-key
     ...
     "dss" 'denote-silo-open-or-create
     "dsn" 'denote-silo-create-note
     "dsd" 'denote-silo-dired
     ...
     "rr" '((lambda () (interactive) (load-file (expand-file-name "init.el" user-emacs-directory))) :which-key "Reload init.el")
     "re" 'eval-buffer
